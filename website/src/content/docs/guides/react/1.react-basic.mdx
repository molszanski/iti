---
title: Basic Usage with React
description: Complete example of using ITI with React
slug: with-react/react-full-intro
sidebar:
  position: 1
---
...
The hook was renamed from `useContainer()` to `useItem()` for better clarity. The old name still works but is deprecated.
:::
...
```tsx title="./src/Profile.tsx"
import { useItem, useItems } from "./_containers/main-app"

// Callback style

function Profile() {
  const [profileDeps, profileDepsErr] = useItems((c) => [c.auth, c.env])
...
// Literal style
function Profile() {
  const [profileDeps, profileDepsErr] = useItems(["auth", "env"])
...
:::tip[SSR Support in v0.8.0]
The `useItems` hook now includes synchronous fallback for better SSR support. It will return cached values synchronously when available, preventing hydration mismatches in Server-Side Rendering scenarios.
:::

:::note[API Naming Update]
The hook was renamed from `useContainerSet()` to `useItems()` for better clarity. The old name still works but is deprecated.
:::
...
const x = generateEnsureItems(() =>
  useItems(["ecommerce", "auth"])
)
...
:::note[API Naming Update in v0.8.0]
The function was renamed from `generateEnsureContainerSet()` to `generateEnsureItems()` for better clarity. The old name still works but is deprecated.
:::
